# FUTURE_CS_01
Cyber Security Internship Program tasks and projects at Future Interns (March - April 2026).
# Future Interns - Cyber Security Internship

## Intern Profile
- **Name:** NADER ABDELGELIL
- **CIN ID:** FIT/MAR26/CS6984
- **Domain:** Cyber Security
- **Location:** Warsaw, Poland

---

## 🛡️ Task 1: Vulnerability Assessment Report
**Status:** In Progress ⏳

### Objective
Perform a professional vulnerability assessment on a live web application (`testphp.vulnweb.com`) to identify security risks, classify them by severity, and provide remediation steps.

### Methodology & Tools
1. **Reconnaissance:** Network scanning and service discovery using **Nmap**.
2. **Vulnerability Scanning:** Automated security testing using **OWASP ZAP**.
3. **Manual Analysis:** Browser DevTools inspection.
4. **Reporting:** Professional documentation designed in **Canva**.

### Progress Checklist
- [x] Create GitHub Repository
- [ ] Network Scanning (Nmap) - *In Progress*
- [ ] Vulnerability Identification
- [ ] Risk Classification
- [ ] Final PDF Report Upload
